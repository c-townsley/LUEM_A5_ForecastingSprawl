...
The raster files were still very large, so here, we down sampled them to make their size more manageable and then clipped them precisely to the MSA boundaries.
...
This distilled, categorical dataset for each year is then aggregated to our fishnet of the Atlanta MSA. 
...
#make ggplot chart at some point, not working yet though
#ggplot() +
 # geom_histogram(data=rast(development_change) %>% na.omit, 
             # aes(x,y,fill=as.factor(value))) +
...
Calculate how far each fishnet cell is from a highway. Convert highway layer to raster then convert raster to points then mean distance from highway points is calculated by grid cell.
...
dat %>%
  dplyr::select(distance_highways,lagDevelopment08,development_change) %>% # development_change instead of lc_change?
  gather(Variable, Value, -development_change, -geometry) %>%
  ggplot(., aes(development_change, Value, fill=development_change)) + # development_change instead of lc_change (x2)?
