...
The model developed will predict two different scenarios: supply-side and demand-side. The supply-side model will predict how future development might occur following the construction of a new, theoretical highway. Through this scenario, we attempt to ascertain the relationship that highway development has with urban sprawl. In the demand-side model, we attempt to understand the relationship that population growth has with urban sprawl. We predict the demand for development according to population growth, and how this would spatially manifest based on existing land use trends. 
...
The following chunks of code will collect the prerequisite data needed for the model from the two years of study, structure the data in preparation for modeling, and run the model supply- and demand-side based on the study years' data to project for 2030. 
...
We then visaulize the population for the two years by census tracts for the two years.
...
We then calculate how far each fishnet cell is from a highway. We do this by converting the highway layer to raster, converting the raster to points, and then calculating the mean distance from highway for each grid cell.
...
#try natural breaks (manual)
```

# Create Final Dataset

We then aggregate all this data to a final fishnet which we will feed into our model. This fishnet includes columns which indicate, cell-by-cell, if there has been development change, its land use, the cell's distance to highways, and its population in both years.
...
This plot displays distance to highways and the spatial lag of development as continuous variables. This shows us that new development has on average, farther from highways, and from existing development in aggregate, than cells that had no change in land cover.
...
This next plot shows that new development has occurred in cells where the population is greater.  
...
From this conversion table, we see that forested areas in 2008 were the most likely to have been developed by 2019, at 8.85% likelihood of development. This shows that Atlanta has a preference for developing woodlands first, with areas adjacent to those that are already developed as a close second. 

```{r land cover conversion tables, fig.width=10}
...
```{r testing model sensitivity: true/false positives/negatives}
...
> From Assignment: In this scenario, you will "plan" a new large-scale development, like a new highway or public transportation line. Your model will probably use transit proximity as a predictor of land use change -- what happens when you make new infrastructure and then predict the location of new development?
...
#highlight new highway
```

## Scenario 2: Demand-side Change Forecast

> From Assignment: Demand-side change: In this scenario, you will find or approximate 2029 population projections for your study area and then 'distribute' the 'new' population among all the census tracts in the region according to a logic or plan that you devise. You will use your population forecast and 2019 data to predict where new development will occur in 2029. Metropolitan planning organizations (MPOs) publish population forecasts for metro areas. If you can't find one for your area, make some reasonable projection of your own and note that you have done so.

Does the 2009 - 2019 period of development in Atlanta generalize to the 2019 - 2029 period?

Replace the lag development 2008 column with the distance from development in 2019.
...
Distribute projected population across the study area by weighting it proportionally according to a grid cell's 2019 population and then subtracting 2019 pop to get `pop_change`. Then, use Model XX to predict development for 2029 based on the projected population change and lag development features.
...
Now that we have an idea of where development demand is most likely to occur in 2029, we compare predicted demand against environmentally sensitive land uses to figure out which land uses we recommend be allocated for development and which be reserved for conservation.

Let's revisit how each kind of land cover was distributed in 2019.
...
We create an indicator layer to show grid cells where forest or wetlands were lost to development between 20098 and 2019.
...
We then group areas where the `wetlands19` and `forest11` rasters are contiguous to create a `sensitive_regions` layer containing all sensitive regions with an area greater than 1 acre.
...
Next we summarize a series of supply and demand metrics by county. We use this to better understand each county's suitability for development.
...
```{r plot county specific metrics, fig.width=12, fig.height=10}
county_specific_metrics %>%
...
![](paste(dir_ct, 'Images/Satellite_HallCounty.png', sep='/'))
...
Let's look more closely at Hall County and Calyton County to see what these traits look like spatially.
...
**Note:** we do not include forest in the "not suitable for development" category in these maps due to the large amount of tree cover in the Atlanta region.
