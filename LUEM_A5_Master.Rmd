...
```{r land cover conversion tables}
...
```{r linear models and plotting their R sqaured values}
...
```{r confusion metrics}
#I think we are going to have to make the all the lc_change whatever it is for the relevant dataset for 2019
...
#highways_addition <- st_read("C:/Users/Lindsey/Desktop/CPLN675_Land_Use_Modeling_Desktop/Final/LUEM_A5_Repo/Data/shapefile/AtlantaMetro_Major_Roads_ADDITION/Highways_addition_Merge.shp")
#highways_add <- st_write(highways_addition,"C:/Users/Lindsey/Desktop/CPLN675_Land_Use_Modeling_Desktop/Final/LUEM_A5_Repo/Data/geojson/highways_add.geojson")

highways_add <- st_read("https://raw.githubusercontent.com/c-townsley/LUEM_A5_Repo/d9859fc1069dd89d44fcaf345e722a9bf601a550/Data/geojson/highways_add.geojson") %>% 
st_transform(st_crs(metro_counties)) %>%
  st_intersection(metro_counties) %>% 
  filter(FEATURE_TY == "State Highway")## This is the merged file with the new road as part of the network

highways_add <- highways_add %>% 
st_transform(st_crs(metro_counties)) %>%
  st_intersection(metro_counties) %>% 
  filter(FEATURE_TY == "State Highway")
