...
ggplot() +
  geom_sf(data=Atlanta_MSA) +
  geom_raster(data=rast(lc_2019r) %>% na.omit, 
              aes(x,y, fill=as.factor(value))) +
    scale_colour_manual(values = c(gray, highlight),
                        labels=c("False","True"),
                        name = "") +
  geom_sf(data=metro_counties,
          ...
          fill = "transparent",
          linewidth = .5) +
  labs(title="Developed Areas (2019)", subtitle='Atlanta Metro Area') + #IS THIS ACTUALLY A MAP OF DEVELOPMENT LAND USE CHANGE?
  mapTheme
```
