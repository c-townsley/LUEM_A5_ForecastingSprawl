...
ggplot() +
  ...
  geom_point(data=AtlantaMSA_fishnet, 
             aes(x=xyC(AtlantaMSA_fishnet)[,1], y=xyC(AtlantaMSA_fishnet)[,2], 
                 colour=factor(ntile(lagDevelopment08,3))), size=1.5) +
...
dat %>%
  dplyr::select(distance_highways,lagDevelopment,lc_change) %>% # development_change instead of lc_change?
  gather(Variable, Value, -lc_change, -geometry) %>%
  ggplot(., aes(lc_change, Value, fill=lc_change)) + # development_change instead of lc_change (x2)?
