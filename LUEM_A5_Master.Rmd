...
#writeRaster(lc_2008, filename=file.path("C:/Users/ctown/OneDrive - PennO365/Classes/Classes_Sem4_2023Spring/CPLN 675_Land Modeling/Assignments/A5_SprawlForecasting/LUEM_A5_Repo/Data/raster", "lc2008_sml.tif"), format="GTiff", overwrite=TRUE)

#writeRaster(lc_2019, filename=file.path("C:/Users/ctown/OneDrive - PennO365/Classes/Classes_Sem4_2023Spring/CPLN 675_Land Modeling/Assignments/A5_SprawlForecasting/LUEM_A5_Repo/Data/raster", "lc2019_sml.tif"), format="GTiff", overwrite=TRUE)

#Load in downsized rasters
lc_2008 <- raster("https://raw.githubusercontent.com/c-townsley/LUEM_A5_Repo/bcec3a195528bb480b2af74a90837472fb9e8acc/Data/raster/lc2008_sml.tif") 
lc_2019 <- raster("https://raw.githubusercontent.com/c-townsley/LUEM_A5_Repo/bcec3a195528bb480b2af74a90837472fb9e8acc/Data/raster/lc2019_sml.tif") 

#set projections
lc_2008 <- projectRaster(lc_2008, crs = crs(rast_projection))
lc_2019 <- projectRaster(lc_2019, crs = crs(rast_projection))
```

The raster data is then clipped with the Atlanta MSA boundary.

```{r}
#crop rasters
lc_2008_crop <- crop(lc_2008, extent(Atlanta_MSA))
lc_2019_crop <- crop(lc_2019, extent(Atlanta_MSA)) 

#mask rasters
lc_2008_crop <- raster::mask(x = lc_2008_crop, mask = Atlanta_MSA)
lc_2019_crop <- raster::mask(x = lc_2019_crop, mask = Atlanta_MSA)
```
...
```{r turn rasters into fishnet dataframe}
...
ggplot() +
  ...
  scale_colour_manual(values = palette5,
                      ...
                      name="Quintile\nBreaks") +
  geom_sf(data=metro_highways, colour = "red") +
...
```{r development spatial lag}
thisFishnet$lagDevelopment <-
    nn_function(xyC(thisFishnet),
                xyC(filter(aggregatedRasters,developed==1)),
                2)

ggplot() +
  geom_sf(data=metro_counties) +
  geom_point(data=thisFishnet, 
             aes(x=xyC(thisFishnet)[,1], y=xyC(thisFishnet)[,2], 
                 colour=factor(ntile(lagDevelopment,5))), size=1.5) +
  scale_colour_manual(values = palette5,
                     labels=substr(quintileBreaks(thisFishnet,"lagDevelopment"),1,7),
                     name="Quintile\nBreaks") +
...
ggplot() +
  ...
  geom_point(data=thisFishnet, 
             aes(x=xyC(thisFishnet)$x, y=xyC(thisFishnet)$y, colour=lc_change)) +
  scale_colour_manual(values = palette2,
                      labels=c("No Change","New Development"),
                      name = "") +
  labs(title = "Land Cover Development Change", subtitle = "As fishnet centroids") +
