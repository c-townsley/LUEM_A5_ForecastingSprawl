...
Now, we use the predicted values from Model 6 to create a histogram of the model's test set predicted probabilities. This shows us that a high density of cells have roughly a 40% likelihood of development.
...
At the first two thresholds specified here, 5% and 17%, we can see that the 5% threshold very accurately predicts the "true negatives," where development does not occur. The threshold of 17%, however, performs better on the "true positive" rate of predicted v. actual development, that has occurred. We found that the best balance between the trade-offs of sensitivity and specificity was shown at a threshold of around 34%. We chose 34% because it gives us the highest overall accuracy, and allows us to keep both our sensitivity and specificity over 50%. 
