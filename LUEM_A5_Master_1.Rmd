...
This section uses data from our first time period to create a model to predict land use change. We will create seven logistic regression models to predict development change between 2008 and 2019.
...
Model7 <- glm(development_change ~ wetlands08 + forest08  + farm08 + otherUndeveloped08 + lagDevelopment08 + distance_highways, 
              family="binomial"(link="logit"), data = datTrain) 
...
We then run the dataset through Model 7 which is agnostic to population change, but responsive to land cover type, the spatial lag of development, and distance to highways. Because population is excluded, this model is effective in showing a true response to supply-side forces.

```{r}
dat_highway <-
  dat %>%
  #predict for 2029
    mutate(predict_2029.distance_highways = predict(Model7,. , type="response"))
